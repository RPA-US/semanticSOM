# Base comes from commit a95907c
import base64
import re
import requests
...
import os
from src.models.models import QwenVLModel, TextModel
...
def qwen_vl_plus(image_path, prompt, api, temperature, top_p):
    ...
    response = dashscope.MultiModalConversation.call(
        model="qwen-vl-plus", messages=messages, temperature=temperature, top_p=top_p
    )
    ...
    if response.status_code == HTTPStatus.OK:
        ...
        raise Exception(
            f"Request failed with code {response.status_code}: {response.message}"
        )
...
def qwen_vl_max(image_path, prompt, api, temperature, top_p):
    ...
    response = dashscope.MultiModalConversation.call(
        model="qwen-vl-max", messages=messages, temperature=temperature, top_p=top_p
    )
    # The response status_code is HTTPStatus.OK indicate success,
    # otherwise indicate request is failed, you can get error code
    # and message from code and message.
...
def local_qwen_vl(
    image_path: str, prompt: str, _, temperature: float, top_p: float
) -> str:
    model = QwenVLModel(model_name="Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4")
    image: Image.Image = Image.open(image_path)
    return model(prompt=prompt, image=image)


def local_deepseek_api_14b(
    image_path: str, prompt: str, _, temperature: float, top_p: float
) -> str:
    model = TextModel(
        model_name="deepseek-r1-distill-qwen-14b",
        openai_server="http://127.0.0.1:1234/v1",
        api_key="lm-studio",
    )
    res = model(prompt=prompt, max_tokens=1024, temperature=temperature, top_p=top_p)
...
if __name__ == "__main__":
    qwen_vl_plus(
        "C:\\apa\\BPMExtension\\input\\images\\Captura de pantalla (147).png",
